...
- Atrito: Que desacelera o movimento do objeto ao longo da superfície.
...
- **ball.py**: A classe Ball representa um corpo de teste que interage com o ambiente simulado. Ela gerencia a posição, velocidade e estado do objeto em resposta às forças aplicadas, como gravidade e atrito, além de detectar e reagir a colisões com a curva de Bézier. A classe também permite interação do usuário, como arrastar o corpo com o mouse e resetar sua posição. Visualmente, exibe animações diferentes dependendo do estado do objeto: rolando, parada ou levantada.
...
### Atrito  
O atrito, no contexto do código, é modelado como uma força proporcional à velocidade tangencial do objeto. Após projetar a velocidade no vetor tangente da curva, a função `update` aplica o atrito pela multiplicação do vetor velocidade por um fator redutor, `self.velocity *= (1 - FRICTION)`. Esse cálculo simula a resistência que desacelera o movimento ao longo da curva. O fator de atrito (`FRICTION`) também pode ser ajustado em tempo real pelo usuário. Ele pode ser ajustado dinamicamente por um controle deslizante (`slider2`). No loop principal, o atrito é recalculado com base na porcentagem definida pelo slider (`friction = slider2.get_percentage() / 200`). Essa força de resistência é aplicada ao vetor de velocidade após projetar o movimento do objeto no vetor tangente da curva de Bézier.  

### Interação com a Curva  
A interação entre o corpo e a curva de Bézier é central para a simulação e é gerenciada em várias etapas:  

1. **Detecção do Ponto de Contato:**  
   A posição mais próxima do corpo na curva é determinada pela função `closest_point_and_tangent` da classe `BezierCurve`. Essa função amostra pontos na curva e calcula a menor distância até a posição do corpo.  

2. **Ajuste de Posição:**  
   Quando a distância do corpo até a curva é menor que o raio do corpo, a posição da corpo é ajustada para evitar interpenetração, utilizando o vetor normal ao ponto de contato.  

3. **Projeção da Velocidade:**  
   Após corrigir a posição, a velocidade da corpo é projetada no vetor tangente ao ponto de contato na curva, simulando deslizamento suave. Em seguida, o efeito do atrito é aplicado à velocidade projetada.  
...
- Atualiza a posição e velocidade da corpo com base nas forças aplicadas e na interação com a curva de Bézier.  
- Renderiza os elementos na tela, incluindo a corpo, a curva de Bézier e os controles.  
